# todo_rules_6.md — Design review checklist: RULES.md §6 Cross-configuration behavior

> **Template, not a live todo.** At the start of a **Design-mode** review,
> copy this file into `bin/`:
>
>     cp llm/todo_rules_6.md bin/todo_rules_6.md
>
...
>   immediately after (S3); keep the `bin/todo_rules_6.md` file and the
...
> - **Delete `bin/todo_rules_6.md` once every checkbox below is
...
Source: `llm/RULES.md` §6 "Cross-configuration behavior".
...
- [ ] §6 — No reliance on implementation-defined behavior (e.g.
      equal-element order after `std::sort`; `std::stable_sort` if order
      matters)
- [ ] §6 — No UB missed by CI sanitizers (e.g. narrowing float→int) — CI
      runs ASan/UBSan
