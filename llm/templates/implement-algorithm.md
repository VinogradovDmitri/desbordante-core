...
> Workflow (todo files, phases): `llm/PLAN.md`. Requirements: `llm/RULES.md`.
> Commands (build, test, snapshot, format): `llm/DEVELOPMENT.md`.
> Performance targets additionally follow `llm/PERFORMANCE.md` and the
> measurement protocol (`llm/PLAN.md` §5).

## 1. Spec (fill before writing any code)

- Name: `<algo>` (snake_case); class: `<AlgoClass>`; pattern: `<fd|ucc|dc|ar|md|nd|rfd|od|ind|…>`
- Paper: `<docs/papers/... or external reference>`
- Primitive, one sentence: `<what it discovers>`
- Options: `<name — type — default — meaning>`
- Output pattern object: `<RULES.md §17 — what the user must be able to do with the result>`
- Format to match (past example): `<an existing algorithm/example to mirror,
  e.g. examples/basic/mining_<existing>.py and its test/snapshot layout>`
...
- [ ] Unit tests `src/tests/unit/test_<algo>.cpp` via `desbordante_add_test`
...
**Conditional loop:** if a check fails → fix → re-run starting from the first
failed check; never skip a check and never report it as passed when it was
not run.

- [ ] Build: `<command>` → `<result>`
- [ ] Targeted tests: `ctest --test-dir build -R "<algo-regex>"` → `<result>`
...
- [ ] Second-pass review: a fresh reviewer pass (or second model) checks the
      final output against the §1 acceptance criteria → `<findings fixed /
      explicitly waived>`
